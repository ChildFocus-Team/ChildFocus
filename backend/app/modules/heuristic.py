...
What this does:
  ...
  - Used by classify.py to compute the final OIR score

Calling styles:
  1. Pass a pre-sampled dict (from sample_video()) — avoids re-downloading
     compute_heuristic_score(sample_dict)
  2. Pass a video_id string — will call sample_video() internally
     compute_heuristic_score("dQw4w9WgXcQ", thumbnail_url="https://...")
...
def compute_heuristic_score(video_id_or_sample, thumbnail_url: str = "") -> dict:
    ...
    Args:
        video_id_or_sample:
            - str:  YouTube video ID or URL — will call sample_video() internally
            - dict: Already-sampled result from sample_video() — skips re-download
        thumbnail_url:
            Optional thumbnail URL (only used when video_id_or_sample is a str)
    ...
    """
    # ── Accept either a pre-sampled dict or a video_id string ─────────────────
    if isinstance(video_id_or_sample, dict):
        result = video_id_or_sample          # already sampled — skip re-download
    else:
        result = sample_video(video_id_or_sample, thumbnail_url)
